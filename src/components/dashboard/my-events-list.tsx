...
export function MyEventsList({ registrations }: MyEventsListProperties): React.ReactNode {
  ...
  return <div className="my-events-list">{registrations.map((registration, index) => {
    if (registration.event === null) {
      return <MotionEventCard delay={index * 0.08} key={registration.id}><article className="surface my-event-card"><div><span className="status-pill status-pill--past">HISTORIAL</span><h2>Evento no disponible</h2><p>El evento asociado ya no está disponible para consultar públicamente.</p></div><span className="my-event-card__status">{registrationStatusLabels[registration.status]}</span></article></MotionEventCard>;
    }

    const event = registration.event;
    return <MotionEventCard delay={index * 0.08} key={registration.id}><article className="surface my-event-card"><div className="my-event-card__date"><strong>{new Intl.DateTimeFormat("es-EC", { day: "2-digit", timeZone: "America/Guayaquil" }).format(new Date(event.starts_at))}</strong><span>{new Intl.DateTimeFormat("es-EC", { month: "short", timeZone: "America/Guayaquil" }).format(new Date(event.starts_at)).replace(".", "").toUpperCase()}</span></div><div className="my-event-card__content"><span className={`status-pill status-pill--${event.status.toLowerCase()}`}>{event.status === "ACTIVE" ? "EN CURSO" : event.status === "PLANNED" ? "PRÓXIMAMENTE" : "FINALIZADO"}</span><h2>{event.title}</h2><p><CalendarDays size={15} /> {formatEventDate(event.starts_at)}</p><p><MapPin size={15} /> {eventModalityLabels[event.modality]} · {event.location}</p><span className="my-event-card__status">{registrationStatusLabels[registration.status]}</span></div>{event.is_published ? <Link aria-label={`Ver ${event.title}`} className="icon-button" href={`/eventos/${event.slug}`}><ExternalLink size={16} /></Link> : null}</article></MotionEventCard>;
  })}</div>;
}

    ...
    <div className="page-shell">
      ...
      <main className="content-wrap event-detail">
        ...
        <div className="event-detail__grid">
          ...
          <aside className="surface event-detail__section"><p className="eyebrow">SPEAKERS</p><h2>Personas que participan</h2>{event.speakers.length === 0 ? <p className="event-detail__muted">Los speakers se anunciarán próximamente.</p> : <div className="speaker-list">{event.speakers.map((speaker) => <article key={speaker.id}><span>{speaker.name.slice(0, 1).toUpperCase()}</span><div><h3>{speaker.name}</h3><p>{speaker.role_title ?? "Speaker invitado"}</p>{speaker.bio === null ? null : <small>{speaker.bio}</small>}</div></article>)}</div>}</aside>
        </div>

...
export default async function AdminContentPage({ searchParams }: AdminContentPageProperties): Promise<React.ReactNode> {
  ...
  return (
    <section className="admin-section">
      ...
      <form action={createTeamMemberAction} className="admin-form surface admin-social-create"><div className="admin-form__heading"><div><p className="eyebrow"><Plus size={14} /> NUEVO MIEMBRO</p><h2>Añadir perfil</h2></div></div><div className="admin-form__grid"><label>Nombre<input maxLength={80} minLength={2} name="name" required /></label><label>Cargo<input maxLength={100} minLength={2} name="roleTitle" required /></label><label className="admin-form__wide">Descripción<textarea maxLength={600} minLength={10} name="description" required rows={3} /></label><label className="admin-form__wide">URL de fotografía HTTPS<input name="imageUrl" placeholder="https://..." type="url" /></label><label>Orden<input defaultValue="0" max="1000" min="0" name="sortOrder" required type="number" /></label><label className="admin-form__checkbox"><input defaultChecked name="active" type="checkbox" /> Perfil visible</label></div><div className="admin-form__footer"><SubmitButton className="button button--primary" pendingLabel="Añadiendo…"><Plus size={15} /> Añadir miembro</SubmitButton></div></form>
      <div className="admin-social-list">{teamMembers.map((member) => {
        ...
        return <article className="surface admin-social-card" key={member.id}><form action={updateAction} className="admin-form"><div className="admin-form__heading"><div><p className="eyebrow"><UserRound size={14} /> {member.active ? "VISIBLE" : "OCULTO"}</p><h2>{member.name}</h2></div></div><div className="admin-form__grid"><label>Nombre<input defaultValue={member.name} maxLength={80} minLength={2} name="name" required /></label><label>Cargo<input defaultValue={member.role_title} maxLength={100} minLength={2} name="roleTitle" required /></label><label className="admin-form__wide">Descripción<textarea defaultValue={member.description} maxLength={600} minLength={10} name="description" required rows={3} /></label><label className="admin-form__wide">URL de fotografía HTTPS<input defaultValue={member.image_url ?? ""} name="imageUrl" type="url" /></label><label>Orden<input defaultValue={member.sort_order} max="1000" min="0" name="sortOrder" required type="number" /></label><label className="admin-form__checkbox"><input defaultChecked={member.active} name="active" type="checkbox" /> Perfil visible</label></div><div className="admin-form__footer"><SubmitButton className="button button--secondary" pendingLabel="Guardando…"><Save size={15} /> Guardar</SubmitButton></div></form><form action={deleteAction} className="admin-social-card__delete"><SubmitButton className="button button--danger" pendingLabel="Eliminando…"><Trash2 size={15} /> Eliminar</SubmitButton></form></article>;
      })}</div>
    </section>
  );
}
